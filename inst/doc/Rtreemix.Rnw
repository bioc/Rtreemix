...
\title{\vspace*{-6ex} The mutagenetic trees mixture models in the world of {\bf Rtreemix}}
...
framework for modeling multiple paths of ordered accumulation of permanent genetic changes.
They were used to model \texttt{HIV} progression characterized by accumulation
of resistance mutations in the viral genome under drug pressure \cite{Beer12005} and
cancer progression by accumulation of chromosomal aberrations in
tumor cells \cite{Rahn2005}. Each path captures a possible route of disease development.
From the mixture model, a genetic progression score (\texttt{GPS})
can be computed for each patient \cite{Rahn2005} that gives an estimate of the
progression of the disease process and can be used for specifying therapies
or estimating survival times of the patients.
Both the mixture model itself and the derived \texttt{GPS} values were shown to improve the
interpretation of disease progression and to have predictive power for estimating the drug resistance
in \texttt{HIV} or the survival time in cancer.
...
and estimation of the GPS values. Furthermore, it introduces new functionality and
some improvements of the \texttt{C/C++} implementation. The available functions are presented in Table 3.2.
...
of a set of genetic events in a group of patients. Each pattern corresponds to a single patient. The set of genetic
...
\textit{hiv.data.RData} in the \textit{data} folder of the package. Therefore, we first load the data.
<<results = hide>>=
data(hiv.data)
...
Having a set of profiles for a group of patients suffering from a certain disease
...
@
It is assumed that mixture models with at least two components always have the noise (star) component as a first component. When only one tree component is fitted to the given data it can be a star or a nontrivial tree component based on the choice of the parameter \texttt{noise}.
...
The mutagenetic trees mixture provides a probability distribution on the set of all possible profiles for a specified set of genetic events.
...
In the \texttt{Rtreemix} package the method \texttt{gps} calculates the \texttt{GPS} of a given set of genetic profiles
...
to a random model than to the fitted model with respect to the chosen model attribute. Details and examples for the stability analysis of a
mutagenetic trees mixture models are given in the \textit{doc} folder of the package in the PDF file \textit{ExtendedVignette}. 
